...
function CrestLogo({ className = "" }: { className?: string }) {
  return (
    <div className={`relative flex items-center justify-center drop-shadow-md ${className}`}>
      <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full text-taupe/90" fill="none" stroke="currentColor">
        <circle cx="50" cy="50" r="48" strokeWidth="0.7" />
        <circle cx="50" cy="50" r="40" strokeWidth="0.7" />
        <g strokeWidth="0.4" opacity="0.7">
          ...
          <polygon points="26,8 74,8 92,26 92,74 74,92 26,92 8,74 8,26" />
        </g>
      </svg>
      <div className="absolute inset-0 flex flex-col items-center justify-center text-umber font-serif font-light">
        <span className="text-[10px] sm:text-[42px] md:text-[56px] leading-none mt-2 drop-shadow-sm">J</span>
        <div className="w-3 sm:w-12 md:w-14 h-[0.5px] bg-taupe/80 my-1 drop-shadow-sm" />
        <span className="text-[10px] sm:text-[42px] md:text-[56px] leading-none drop-shadow-sm">S</span>
      </div>
    </div>
  );
}
...
function RSVPForm() {
  ...
  const [guests, setGuests] = useState<GuestEntry[]>([{ name: "", meal: "non-veg", liquor: "no" }]);
  ...
  useEffect(() => {
    if (partyType === "individual") {
      setGuestCount(1);
      setGuests((prev) => [prev[0] ?? { name: "", meal: "non-veg", liquor: "no" }]);
      return;
    }
  ...
  useEffect(() => {
    ...
    setGuests((prev) => {
      ...
      while (next.length < desiredCount) next.push({ name: "", meal: "non-veg", liquor: "no" });
      return next;
    });
      ...
      <form onSubmit={submit} className="space-y-4 md:space-y-4 px-1 md:px-2">
        ...
        <div className="space-y-2">
          {(isAttending ? guests : [guests[0]]).map((guest, idx) => (
            <div key={idx} className="grid grid-cols-1 md:grid-cols-[1fr_100px_100px] gap-2">
              <input
                data-no-flip
                value={guest?.name ?? ""}
              ...
              <select
                data-no-flip
                disabled={!isAttending}
                value={guest?.liquor ?? "no"}
                onChange={(ev) => updateGuest(idx, { liquor: ev.target.value as "yes" | "no" })}
                className={`w-full rounded-xl border border-sage/20 bg-white/60 px-3 py-2.5 text-xs text-zinc-700 outline-none ${!isAttending ? "opacity-60" : ""
                  }`}
              >
                <option value="yes">Liquor</option>
                <option value="no">No Liquor</option>
              </select>
            </div>
          ))}
        </div>
    ...
    >
          ...
          >
            ...
            >
              ...
              {guestPrefix && guestName && (
                <p className="mt-4 text-lg md:text-2xl text-umber/80 font-serif italic">
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4 md:gap-16 mt-4 md:mt-8 relative w-full px-2">
            <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 h-32 bg-sage/5 blur-3xl rounded-full" />

            <motion.h2 whileHover={{ scale: 1.05 }} className="script text-[18vw] sm:text-6xl md:text-9xl text-sage drop-shadow-lg relative z-10 leading-none">
              Janani
            </motion.h2>

            <div className="relative flex items-center justify-center shrink-0 my-2 sm:my-0">
              <div className="h-px w-6 md:w-24 bg-sage/20 hidden md:block" />
              <div className="relative mx-1 md:mx-4">
                <span className="serif italic text-3xl sm:hidden text-taupe/60 mr-2">&amp;</span>
                <Heart className="hidden sm:block text-sage/40 w-5 h-5 sm:w-7 sm:h-7 md:w-10 md:h-10 animate-pulse" fill="currentColor" />
                <motion.div
        ...
        <div className="flex justify-center w-full mb-8 mt-28 sm:mt-32 md:mt-48">
          {!isOpened ? (
              ...
              >
                ...
                <div className="relative bg-paper rounded-[1.6rem] md:rounded-[2rem] shadow-[0_-20px_60px_rgba(0,0,0,0.16),0_10px_30px_rgba(0,0,0,0.08)] border border-sand/35 overflow-hidden">
                  ...
                  <div className="relative z-10 px-4 pt-4 pb-3 sm:px-6 sm:pt-7 sm:pb-7 md:px-10 md:py-8 flex flex-col items-center text-center gap-0 sm:gap-2 md:gap-3">
                    ...
                    >
                      <CrestLogo className="w-16 h-16 sm:w-32 sm:h-32 md:w-40 md:h-40" />
                    </motion.div>

                    {/* hosting families */}
                    <div className="space-y-0.5">
                      <p className="serif text-[9px] sm:text-[10px] md:text-[13px] uppercase tracking-[0.3em] text-umber font-normal leading-relaxed text-center">
                        MR GNANATHILAKA<br />&amp;<br />MRS VIDANAPATHIRANA
                      </p>
                      <p className="text-[7px] sm:text-[8px] md:text-[9px] uppercase tracking-[0.25em] text-taupe font-medium my-2">
                        TOGETHER WITH
                      </p>
                      <p className="serif text-[9px] sm:text-[10px] md:text-[13px] uppercase tracking-[0.3em] text-umber font-normal leading-relaxed text-center">
                        MR. JAYASINGHE (LATE)<br />&amp;<br />MRS. WANIGASINGHE
                      </p>
                    </div>

                    <p className="serif mt-3 mb-2.5 sm:mb-0 text-[10px] sm:text-[11px] md:text-[14px] uppercase tracking-[0.2em] text-taupe/80 font-normal leading-relaxed max-w-[200px] md:max-w-xs">
                      {guestPrefix && guestName ? (
                        <>
                          WE CORDIALLY INVITE<br />
                          <span className="text-[12px] sm:text-[14px] md:text-[16px] text-sage font-bold block my-1">
                            {guestPrefix} {guestName}
                          </span>
                          TO CELEBRATE THE MARRIAGE OF THEIR CHILDREN
                        </>
                      ) : (
                        "REQUEST THE HONOR OF YOUR PRESENCE TO CELEBRATE THE MARRIAGE OF THEIR CHILDREN"
                      )}
                    </p>
              ...
              <div className="w-full h-full bg-[#F5EFE0] p-2 md:p-8 flex flex-col items-center justify-center text-center space-y-2 md:space-y-4 relative group">
                ...
                <div className="relative z-10 space-y-2 md:space-y-8 scale-[0.9] md:scale-100">
                  ...
                  <div className="flex flex-col items-center">
                    <p className="text-[8px] md:text-xs uppercase tracking-[0.4em] text-zinc-400 font-black mb-1 md:mb-2">Friday</p>
                    <div className="relative inline-block px-6 md:px-8 py-1 md:py-2 border-y border-sage/30">
                      ...
                      <motion.div
                        ...
                        transition={{ repeat: Infinity, duration: 2 }}
                        className="absolute -top-1 -right-1 text-[#E6A8D7]"
                      >
                        <Sparkles size={12} className="md:w-4 md:h-4" />
                      </motion.div>
                    </div>
                    <p className="serif text-sm md:text-2xl font-light tracking-[0.2em] mt-2 md:mt-3">OCTOBER</p>
                  </div>
              ...
              front={
                <div className="w-full h-full bg-[#F5EFE0] p-6 flex flex-col justify-center items-center text-center relative group overflow-hidden">
                  ...
                  <div className="relative z-10 space-y-3 md:space-y-6">
                    ...
                    >
                      <CrestLogo className="w-[25vw] h-[25vw] sm:w-[180px] sm:h-[180px] md:w-[240px] md:h-[240px] opacity-90" />
                    </motion.div>
                  </div>
                </div>
              }
                ...
                <div className="w-full h-full relative group">
                  ...
                  <div className="absolute top-6 right-6 md:top-10 md:right-10 bg-white/60 backdrop-blur-md p-4 md:p-8 border border-white/60 rounded-2xl group-hover:bg-white/80 transition-all duration-700 shadow-xl">
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      onClick={() => window.open("https://maps.app.goo.gl/dmmvshQwDAaVWgpW7", "_blank")}
                  ...
                  <div className="absolute bottom-6 left-6 md:bottom-10 md:left-10 text-sage flex items-center gap-3 bg-white/60 backdrop-blur-md px-4 py-2 rounded-full border border-white/60 shadow-lg">
                    ...
                    <p className="serif text-[10px] md:text-sm tracking-[0.2em] font-bold uppercase">Shangri-La</p>
                  </div>
                </div>
              }
              back={
                <>
                  ...
                  <h4 className="serif text-2xl md:text-4xl text-sage mb-2 md:mb-4 text-center">Hotel Shangri-La Hambantota</h4>
                  <p className="text-[10px] md:text-sm text-zinc-500 uppercase tracking-widest leading-loose mb-4 md:mb-6 text-center">
                    Hotel Shangri-La
                    <br />
                    Hambantota
                  </p>
